...
本文描述 **QUAP 0.1.0** 已经实现的系统架构，而不是规划中的能力。概览见[系统框架](系统框架.md)，部署与开通步骤见[部署说明](部署说明.md)，本次仓库内验证结果见[系统验证报告](系统验证报告.md)，后续计划见[内部路线图](内部路线图.md)。架构图：[architecture.svg](architecture.svg)。
...
| `research` | `worker --role research` | 钉住水位后的个人因子诊断与日频回测 |
...
| `src/quant_platform/analysis` | 原生指标、篮子观察、筛选；版本 `0.1.0:native-2` |
...
模式由 `schema.sql` 建立，Alembic `0002` 追加恢复字段，`0003` 追加告警外盒和个人读写角色。破坏性降级被禁用，回退靠备份恢复。
...
5. 盘中分析只对未暂停篮子写分钟观察。覆盖率不足、过期或未核验的观察不能触发告警，也不能重新武装告警边沿。缺失值 `None` 直接跳过。
6. 日频分析在领取后把证券主数据、日历、生效篮子、规则、历史窗口、分析版本和 dataset 水位写入 `jobs.progress.snapshot`。重试复用该快照，不受中途改名、归档或新 K 线修订影响。日历或 K 线缺口把指标标为 `calendar_gaps`，不会用零填充连成假时间序列。
7. 筛选给出可解释候选（20 日收益与波动的加权分位）。审批 API 只接受该报告候选的无重复子集，并创建下一交易日生效的新篮子。筛选本身不改篮子。
...
| GET | `/api/v1/reports` | stock / basket / screen |
...
看板工作区：Operations（10 秒刷新）、Baskets、Stocks、Candidates、Reports。登录只使用操作员令牌。断线时明确提示保留数据不是实盘。
...
必需能力：`stock_basic`、`trade_cal`、`daily`、`adj_factor`、`rt_k`。可选：`index_daily`、`suspend_d`。`doctor` 按真实访问和返回结构探测；空的必需响应不算通过。付费账户本身不证明接口权限、再分发权利或延迟协议。
