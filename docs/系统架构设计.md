...
| `qualification` / `maintenance` / `backup` / `notify` | operations | 资格采样、分区保留、pg_dump、每日 09:00 研究邮件 |
| `qlib_export` | qlib | 可选不可变导出 |

调度器每个 tick 尝试领导锁。合格交易时段内，若轮询未暂停且 `rt_k` 未阻塞，则在上一轮完成后等待 `quote_seconds`（默认 30，范围 15–3600）再入队行情。已跟踪篮子成员和未归档持仓优先；名录新鲜时才会扩展到全市场。每个行情任务最多 200 只证券。每个上海自然日 09:00 后入队 `notify:{day}`；09:00 前不入队，当天晚启动会补跑。邮件正文使用最近一份已完成筛选报告（09:00 多为上一交易日 as-of）和张江高科成本 28 元的规则建议。
...
8. 每个上海自然日 09:00 后发送两封研究邮件：筛选前 3 名，以及张江高科成本 28 元的持仓价位建议。SMTP 未配置则推迟，不改用其它通知通道。
...
| `QUANT_NOTIFY_EMAIL` | `jxiaoping@gmail.com` | 每日研究邮件收件人 |
| `QUANT_SMTP_HOST` | `smtp.gmail.com` | SMTP 服务器 |
| `QUANT_SMTP_PORT` | 587 | `465` 时改用 SMTPS |
| `QUANT_SMTP_USER` | `jxiaoping@gmail.com` | SMTP 登录名 |
| `QUANT_SMTP_PASSWORD` | 空 | Gmail 应用专用密码；空则 `notify` 推迟 |
| `QUANT_SMTP_PASSWORD_FILE` | 空 | 可选密钥文件，覆盖密码环境变量 |
| `QUANT_SMTP_FROM` | 空 | 空则使用 SMTP 用户名 |
